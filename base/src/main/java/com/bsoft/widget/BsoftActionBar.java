...
//		layAction.setBackgroundColor(Color.BLUE);
        ...
        tvMainTitle.setEllipsize(TextUtils.TruncateAt.MARQUEE);//设置当文本过长时（超过tv文本域），用跑马灯的方式显示文本
    ...
    public void setActionTxtColor(int color) {
    ...
    private View inflateAction(Action action) {
        ...
        if (!TextUtils.isEmpty(action.getText())) {
            ...
            if (actionTxtColor != 0) {
                tv.setTextColor(actionTxtColor);
            }
        } else {
